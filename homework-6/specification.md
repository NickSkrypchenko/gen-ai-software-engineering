...
2. **Validation is terminal on failure.** Transactions that fail validation (missing required field, non-positive amount, non-ISO-4217 currency, malformed ISO 8601 timestamp) terminate as `REJECTED_VALIDATION` in `shared/results/` and never reach the Fraud Detector or Compliance Checker.
