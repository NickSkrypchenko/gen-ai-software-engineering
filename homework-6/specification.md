...
- **PII.** `source_account`, `destination_account`, and any name/description are sensitive. Never write them in plaintext to logs — mask account numbers (`ACC-1001` → `ACC-***1`). The audit log stores `transaction_id` and outcome, not raw PII. Masking is log-scoped by design: the audit log and reason strings are masked, while the `shared/` message-envelope payloads intentionally retain the raw account numbers because the denylist/sanctions check matches on them — no PII is ever written to logs.
